...
class _AccountDeletionScreenState extends State<AccountDeletionScreen> {
  ...
  Future<void> _showDeletionConfirmDialog() async {
    String content = '이 작업은 되돌릴 수 없습니다.\n계정과 모든 관련 데이터가 영구적으로 삭제됩니다.';
    
    if (_selectedReason != null) {
      content += '\n\n선택한 탈퇴 사유: $_selectedReason';
    }
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      // 성공 시 진행 다이얼로그 닫기
      if (mounted) {
        CommonLoadingDialog.hide(context); // 진행 다이얼로그 닫기
  ...
  void _showProgressDialog() {
    CommonLoadingDialog.show(
      context: context,
      message: '계정을 삭제하고 있습니다...\n잠시만 기다려주세요.',
    );
  }

  Future<void> _showSuccessDialog() async {
    await CommonConfirmDialog.showComplete(
      context: context,
      title: '삭제 완료',
  ...
  Future<void> _showErrorDialog(String error) async {
    await CommonConfirmDialog.show(
      context: context,
      title: '삭제 실패',
      content: '계정 삭제 중 오류가 발생했습니다.\n다시 시도해주세요.\n\n오류: $error',
      confirmText: '확인',
      showCancelButton: false,
    );
  }
